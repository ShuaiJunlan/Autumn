...
import com.autumnframework.login.controller.BasicController;
...
public class ResourceController extends BasicController {
    ...
    @ResponseBody
    public JSONObject ajaxResourceList(Resource resource){
        return JSON.parseObject(resourceService.selectResourceResultPageList(resource));
    }
    ...
    @ResponseBody
    public ResponseMsg ajaxSaveResource(Resource res){
        try {
            return resourceService.saveOrUpdateResource(res, this.getCurrentLoginName() );
        } catch (Exception e) {
//            log.error("保存用户信息方法内部错误",e);
            return ResponseMsgUtil.returnCodeMessage(BussinessCode.RES_SAVE_ERROR);
        }
    }
